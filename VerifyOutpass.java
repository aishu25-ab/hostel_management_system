...
public class VerifyOutpass extends HttpServlet  
{    
     public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException 
      {  
         ...
         {  
             ...
             ResultSet rs = stmt.executeQuery("select outpass.email, name,year,hostel,roomno,fromdate,todate,fromtime,totime,reason from outpass,student where student.email=outpass.email and status='approved';");  
             ...
             {   email=rs.getString("outpass.email");
                 ...
                 out.println("<tr><td>" + name + "</td><td>" + year + "</td><td>" + hostel + "</td><td>" + roomno + "</td><td>" + fromdate + "</td><td>" + fromtime + "</td><td>" + todate + "</td><td>" + totime + "</td><td>" + reason + "</td><td>" + "<form method=\"POST\" action=\"http://localhost:8080/hms/VerifyOutpassServlet\" id=\"form4\" onsubmit=\"function x(event){event.preventDefault();}\"><input type=\"hidden\" name=\"email\" value=\"" + email + "\"><input type=\"submit\" value=\"Verify\"></form>" + "</td></tr>");
